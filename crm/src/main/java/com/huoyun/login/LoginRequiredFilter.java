...
public class LoginRequiredFilter implements Filter,
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		localSessionAttributeName = this.filterConfig
				.getInitParameter("LOCALE_SESSION_ATTRIBUTE_NAME");
		...
		try {
			...
			if (!saml2Configuration.isSsoEnabled()
					|| httpRequest.getRequestURI().endsWith(
							saml2Configuration.getLocalACSEndpoint()) || isSkip) {

				if (isSkip) {
					logger.info("Request {} is skipping SSO authorization.",
			...
			if (null != session
					&& null != session
							.getAttribute(EndpointsConstatns.SSO_USER_SESSION_ATTR)) {
				chain.doFilter(request, response);
			} else {
				logger.info(
						"Request is not authorized, require for #SSO authorization. Request URL : {}",
				...
				if (PAOSHTTPBinding.isUserAgentSupportECP(httpRequest)) {
					...
					sendPAOSBinding(httpRequest, httpResponse,
							saml2Configuration);
				} else if (httpRequest.getHeader("Accept") != null
						&& httpRequest.getHeader("Accept").contains("json")) {
					logger.info("Unsupported Accept Header {} from request.",
							"json");
					...
					((HttpServletResponse) response).setStatus(401);
						...
						|| httpRequest.getHeader("Accept").contains("html")
						|| httpRequest.getHeader("Accept").contains("*/*")) {
					logger.info(
							"Supported Accept Header {}, sending HTTPPostBinding....",
							httpRequest.getHeader("Accept"));
					sendHTTPPostBinding(httpRequest, httpResponse,
							saml2Configuration);
				} else {
					logger.info("Unsupported Accept Header {} from request.",
							httpRequest.getHeader("Accept"));
	...
	public void sendHTTPPostBinding(HttpServletRequest httpRequest,
			HttpServletResponse httpResponse,
			SAML2Configuration saml2Configuration) throws IOException,
			SAML2ConfigurationException, SAML2Exception, ServletException {
		...
		logger.info("Sending SSO HTTPPostBinding, acsUrl {}, relayState {}.",
				acsUrl, relayState);

		httpResponse.addHeader("MobileNativeSupport", "true");
		SAML2AuthRequest authnRequest = SAML2Authentication.getInstance()
				.createSAML2AuthRequest(spConfiguration,
	...
	protected String getRealyState(HttpServletRequest httpRequest) {
		...
		if ((httpRequest.getScheme().equals("https") && httpRequest
				.getServerPort() != 443)
				|| (httpRequest.getScheme().equals("http") && httpRequest
						.getServerPort() != 80)) {
	...
	public void sendPAOSBinding(HttpServletRequest httpRequest,
			HttpServletResponse response, SAML2Configuration saml2Configuration)
		...
		SAML2AuthRequest samlRequest = SAML2Authentication.getInstance()
				.createSAML2AuthRequest(spConfiguration,
						saml2Configuration.getDefaultIdPName(), acsUrl);
		PAOSHTTPBinding.sendSAML2AuthnRequest(response, paosRequest,
				ecpRequest, relayState, samlRequest);
		httpRequest.getSession().setAttribute(
				EndpointsConstatns.SSO_SAML2_ORIGINAL_REQUEST_ID,
				samlRequest.getID());
	}

	private String getAcsUrl(HttpServletRequest httpRequest,
		...
		sb.append(httpRequest.getContextPath()).append(
				saml2Configuration.localACSEndpoint);
		return sb.toString();
	}
	...
	public static void syncLocale(HttpServletRequest httpRequest) {
		...
		if (StringUtils.isNotBlank(httpRequest.getParameter("locale"))) {
			try {
				locale = LocaleUtils.toLocale(httpRequest
						.getParameter("locale"));
		...
		if (null == locale && null != httpRequest.getSession(false)) {
			locale = (Locale) httpRequest.getSession(false).getAttribute(
					localSessionAttributeName);
			if (locale != null) {
				logger.debug(
