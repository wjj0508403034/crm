...
public class ACSServlet extends HttpServlet implements
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (null != req.getContentType()
					&& req.getContentType().contains(
							SOAPHTTPBinding.SOAP_MIME_TYPE)) {
				ECPResponseMessage ecpResponse = PAOSHTTPBinding
						.extractECPResponseMessage(req);
				String acs = getAcsUrl(req, saml2Configuration);
				String response = ecpResponse.getSAML2Response()
						.getXMLRepresentation().replaceAll("[\\r\\n]*", "");
				SAML2Principal saml2Principal = SAML2Authentication
						.getInstance().validateResponse(spConfiguration,
								response, null, acs);
				...
				SAML2Principal saml2Principal = SAML2Authentication
						...
						.validateResponse(
								spConfiguration,
								SAML2Utils.decodeBase64AsString(req
										.getParameter(HTTPPostBinding.SAML_RESPONSE)),
								null, acs);
	 ...
	 */
	private void addSessionIndex2HttpSession4SOAPType(HttpServletRequest req,
			SAML2Principal saml2Principal) {
