...
public class LogoutTriggerServlet extends HttpServlet{

	private static final long serialVersionUID = 3518368591782236246L;

	private static final Logger logger = LoggerFactory.getLogger(LogoutTriggerServlet.class);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            CookieUtils.removeCookie(req, resp, CookieUtils.DEFAULT_SBO_PATH, CookieUtils.COOKIE_NAME);
            ...
            if (session != null) {
                ...
                if (sessionPrincipal != null) {
                    auditLogger.info("User {} logging out", sessionPrincipal.getNameId());
                    logger.info("[Logout] User {} request to log out.", sessionPrincipal.getName());

                    SAML2LogoutRequest logoutRequest = SAML2Authentication.getInstance().createSLORequest(
                            spConfiguration, sessionPrincipal);
                    ...
                    CookieUtils.removeCookie(req, resp, CookieUtils.DEFAULT_SBO_PATH, CookieUtils.JSESSION_COOKIE_NAME);
                    CookieUtils.removeCookie(req, resp, CookieUtils.DEFAULT_SLD_PATH, CookieUtils.JSESSION_COOKIE_NAME);
                    CookieUtils.removeCookie(req, resp, CookieUtils.DEFAULT_SBO_PATH, CookieUtils.RSESSION_COOKIE_NAME);
                    CookieUtils.removeCookie(req, resp, CookieUtils.DEFAULT_SLD_PATH, CookieUtils.RSESSION_COOKIE_NAME);
                    resp.sendRedirect(sloEndpoint);
                }
            } else {
                String reDirectURL = saml2Configuration.getHomePage() + "?" + LOGOUTPARAM + "=true";
                logger.info("[Logout] Session not found, redirect to {}", reDirectURL);
