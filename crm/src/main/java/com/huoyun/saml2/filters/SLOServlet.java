...
public class SLOServlet extends HttpServlet{
    ...
    private static final Logger logger = LoggerFactory.getLogger(SLOServlet.class);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            parameters.put(HTTPPostBinding.SAML_REQUEST, req.getParameter(HTTPPostBinding.SAML_REQUEST));
            parameters.put(HTTPPostBinding.SAML_RESPONSE, req.getParameter(HTTPPostBinding.SAML_RESPONSE));
            parameters.put(HTTPPostBinding.SAML_RELAY_STATE, req.getParameter(HTTPPostBinding.SAML_RELAY_STATE));

            SLOInfo sloInfo = com.sap.security.saml2.sp.sso.SAML2Authentication.getInstance()
                    .validateSLOMessageHttpBody(spConfiguration, parameters);
            if (sloInfo instanceof SLORequestInfo) {
                handleSLORequest(req, resp, saml2Configuration, spConfiguration, (SLORequestInfo) sloInfo);
            } else if (sloInfo instanceof SLOResponseInfo) {
                handleSLOResponse(req, resp, saml2Configuration, spConfiguration, (SLOResponseInfo) sloInfo);
            }
        } catch (Exception e) {
            throw new ServletException(e);
        }
    ...
    private void handleSLORequest(HttpServletRequest req, HttpServletResponse resp,
            SAML2Configuration saml2Configuration, SAML2SPConfigurationCustom spConfiguration,
            SLORequestInfo sloRequestInfo) throws IOException, SAML2Exception, SAML2ConfigurationException,
            ServletException {
        ...
        for (String sessionIndex : receivedSessionIndexes) {
            if (userSessionIndexes.contains(sessionIndex)) {
                ...
                SAML2LogoutResponse logoutResponse = SAML2Authentication.getInstance().createSLOResponse(
                        spConfiguration, sloRequestInfo);
                ...
                req.getServletContext().getRequestDispatcher("/waiting.jsp").forward(req, resp);
                return;
            }
        }
    ...
    private void handleSLOResponse(HttpServletRequest req, HttpServletResponse resp,
            SAML2Configuration saml2Configuration, SAML2SPConfigurationCustom spConfiguration, SLOResponseInfo sloInfo)
            throws IOException {
        ...
        // it sucks, we have to make this kind of change just to make jmeter work
        String home = saml2Configuration.getHomePage();
        // if(!home.endsWith("/"))
        // home=home+"/";
        resp.sendRedirect(new StringBuilder(home).append("?locale=").append(URLEncoder.encode(localeStr, "utf-8"))
    ...
    private void sendErrorResponse(HttpServletRequest req, HttpServletResponse resp, SAML2SPConfiguration spConfig,
            SAML2ErrorResponseDetails details) throws IOException, SAML2Exception, SAML2ConfigurationException,
            ServletException {
